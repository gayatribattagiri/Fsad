...
export default function Navigation() {
  ...
  const handleLogin = (response) => {
    const welcomeMessage = response.message;
    localStorage.setItem('welcomeMessage', welcomeMessage);
    setIsLoggedIn(true);
    setUsername(welcomeMessage);
  };
...
{isLoggedIn ? (
<>
<Button className="text-sm font-medium text-[#3e7089]" onClick={handleLogout} >
Logout
</Button>
<span className="font-semibold text-[#3e7089]">Hello, {username}</span>
</>
) : (
<Button
className="text-sm font-medium text-[#3e7089]"
...
Login
            ...
            <button
              ...
              className="text-[#3e7089] hover:text-[#1f2937]"
            >
              Cart
            </button>
          </div>
